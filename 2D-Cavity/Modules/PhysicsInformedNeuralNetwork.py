...
def PINN(batch_size, train_dataset, X_collocated, Y_collocated, X_BC, Y_BC, u_BC, v_BC, p_BC, NC, n_hidden_layers, n_hidden_neurons):
    ...
    initializer = tf.keras.initializers.GlorotNormal()
    u_pred = tf.keras.layers.Dense(n_targets, activation='linear', kernel_initializer=initializer, dtype=tf.float64)(x)
    v_pred = tf.keras.layers.Dense(n_targets, activation='linear', kernel_initializer=initializer, dtype=tf.float64)(x)
    p_pred = tf.keras.layers.Dense(n_targets, activation='linear', kernel_initializer=initializer, dtype=tf.float64)(x)
    model = tf.keras.Model(inputs=[input_x, input_y], outputs=[u_pred, v_pred, p_pred],)
    ...
    @tf.function
    def train_step(x, y, x_BC, y_BC, u_BC, v_BC, p_BC):
        with tf.GradientTape() as tape:
        
            # Boundary conditions evaluation
            u_BC_pred = model([x_BC, y_BC], training=True)[0]
            v_BC_pred = model([x_BC, y_BC], training=True)[1]
            p_BC_pred = model([x_BC, y_BC], training=True)[2]            
            loss_BC = loss_fn(u_BC, u_BC_pred) + loss_fn(v_BC, v_BC_pred) + loss_fn(p_BC, p_BC_pred) 
            ...
            with tf.GradientTape(persistent=True) as tape_diff:
                ...
                u_pred = model([x, y], training=True)[0]
                v_pred = model([x, y], training=True)[1]
                p_pred = model([x, y], training=True)[2]
                
                u_x = tape_diff.gradient(u_pred, x)
                u_y = tape_diff.gradient(u_pred, y)
                v_x = tape_diff.gradient(v_pred, x)
                v_y = tape_diff.gradient(v_pred, y)
                p_x = tape_diff.gradient(p_pred, x)
                p_y = tape_diff.gradient(p_pred, y)
            u_xx = tape_diff.gradient(u_x, x)
            u_yy = tape_diff.gradient(u_y, y)
            v_xx = tape_diff.gradient(v_x, x)
            v_yy = tape_diff.gradient(v_y, y)
            ...
            e2 = u_pred*u_x + v_pred*u_y + (1/(100**2))*(p_x) - (1/100)*(u_xx + u_yy)         
            loss_e2 = loss_fn(0, e2)
            
            e3 = u_pred*v_x + v_pred*v_y + (1/(100**2))*(p_y) - (1/100)*(v_xx + v_yy)         
            loss_e3 = loss_fn(0, e3)
        
            # General loss function
            loss = loss_e1 + loss_e2 + loss_e3 + loss_BC
        ...
        return loss, loss_e1, loss_e2, loss_e3, loss_BC
    ...
    for epoch in range(epochs):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            loss_train, loss_e1, loss_e2, loss_e3, loss_BC = train_step(x_batch_train, 
                                                                        y_batch_train, 
    ...
    file1.close()
    u_collocated = model.predict([X_collocated, Y_collocated])[0]
    v_collocated = model.predict([X_collocated, Y_collocated])[1]
    p_collocated = model.predict([X_collocated, Y_collocated])[2]
    u_BC = model.predict([X_BC, Y_BC])[0]
    v_BC = model.predict([X_BC, Y_BC])[1]
    p_BC = model.predict([X_BC, Y_BC])[2]
    return u_collocated, u_BC, v_collocated, v_BC, p_collocated, p_BC