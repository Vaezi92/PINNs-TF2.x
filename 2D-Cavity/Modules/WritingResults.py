...
def Contour(NP, NC, NF, NR, NFR, BCType, BCTitle, IDS, X, Y, Z, NFN, NFW, NFF, NFI, NFO, NFS, X_collocated, Y_collocated, u_collocated, v_collocated, p_collocated, X_BC, Y_BC, u_BC, NX, NY, DA, A, n_hidden_layers, n_hidden_neurons):

    Corn = EdgeToCell(NF, NC, IDS)
    
    file1 = open('Contour_'+str(NC)+'Cell_'+str(n_hidden_layers)+'layers_'+str(n_hidden_neurons)+'neurons'+'.Plt','w')
    file1.write('Variables="x","y","u","v","p" \n')
    file1.write('Zone N = '+str(NP)+'   E = '+str(NC)+'\n')
    file1.write('ZONETYPE=FEQUADRILATERAL DATAPACKING=BLOCK VARLOCATION=([3-5]=CELLCENTERED) \n')
#    file1.write('\n')
    for j in range(NP):
        file1.write(str(X[j])+'\n')
    for j in range(NP):
...
#        file1.write(str(Z[j])+'\n')
    ...
    for j in range(NC):
        file1.write(str(v_collocated[j][0])+'\n')
    for j in range(NC):
        file1.write(str(p_collocated[j][0])+'\n')
    for i in range(NC):
        P1 = Corn[0][i]
        P2 = Corn[1][i]
        P3 = Corn[2][i]
